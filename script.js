...
let isTeam = false;
document.getElementById("game").value = 'single';
class Player {
...
function submit() {
    ...
    xhr.onreadystatechange = function() {
        if (xhr.readyState === 4) {
            ...
            document.getElementById("teamNumber").innerText = "Team number: " + JSON.parse(xhr.responseText).msg.teamNo;
            document.getElementById("password").innerText = "Password: " + JSON.parse(xhr.responseText).password;
        }
    };

    let data = (isTeam && player2) ? JSON.stringify([player1, player2]) : JSON.stringify([player1]);
